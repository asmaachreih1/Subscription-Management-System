...
import { CreditCard, LogIn, Mail, Lock, AlertCircle, Loader2, ArrowRight, Eye, EyeOff } from 'lucide-react';
...
export const LoginPage = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-mesh p-4 sm:p-6 lg:p-8 overflow-hidden relative font-sans">
            ...
            <div className="max-w-md w-full bg-white/40 backdrop-blur-[32px] border border-white/20 rounded-[2.5rem] sm:rounded-[3rem] p-8 sm:p-12 shadow-[0_32px_120px_-20px_rgba(0,0,0,0.06)] relative overflow-hidden">
                {/* Brand Header */}
                <div className="flex flex-col items-center mb-8 sm:mb-12 text-center">
                    ...
                    <p className="text-slate-400 font-black text-[9px] sm:text-[10px] uppercase tracking-[0.3em]">Log In to Your Account</p>
                </div>

                {error && (
                    <div className="bg-rose-50 border border-rose-100 text-rose-500 p-4 sm:p-5 rounded-2xl mb-8 flex items-start gap-3 animate-in fade-in zoom-in-95 duration-300">
                ...
                <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                    <div className="space-y-2.5 sm:space-y-3">
                        ...
                        <div className="relative group">
                            ...
                            <input
                                ...
                                placeholder="example@subflow.com"
                                value={email}
                    ...
                    <div className="pt-2 sm:pt-4">
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-slate-800 hover:bg-blue-600 text-white font-black py-4.5 sm:py-5 rounded-[1.5rem] sm:rounded-[1.8rem] transition-all duration-500 flex items-center justify-center gap-3 shadow-xl shadow-slate-200 active:scale-[0.97] disabled:opacity-50"
                        >
                            {loading ? (
                            ...
                            <span className="text-sm sm:text-base uppercase tracking-widest">{loading ? 'Logging In...' : 'Log In'}</span>
                        </button>
                    </div>
                </form>

                <div className="mt-10 sm:mt-12 text-center border-t border-slate-100 pt-8 sm:pt-10">
                    <p className="text-slate-400 font-bold text-xs sm:text-sm">
                        Don't have an account?{' '}
                        <Link to="/register" className="text-blue-600 hover:text-blue-700 transition-all decoration-blue-500/20 underline decoration-2 underline-offset-8 font-black group inline-flex items-center gap-1">
                            Sign Up
